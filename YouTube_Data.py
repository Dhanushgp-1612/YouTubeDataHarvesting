...
# ========================================   /     Data collection and Storing it to MongoDB    /   =================================== #

# Data Extraction
tab1, tab2, tab3 = st.tabs(['Extract', 'Transform','Analysis'])
with tab1:
    st.header(':violet[Data Extraction]')
    st.write ('(Note:- Extract the data from YouTube API using channel id and upload it to MongoDB database.)')
    channel_id = st.text_input('**Enter channel_id**')
    Get_data = st.button('**Extract data and load to MongoDB**')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyBXVnBlIEAK8rK99EjgFVTT7KndySE-RqE'
...
# ========================================   /     Data Transforming from MongoDB to SQL    /   ========================================== #

with tab2:
    st.header(':violet[Data Transformation]')
    st.write ('''(Note:- Load the data from MongoDb to SQL for querying and analysis.)''')
    ...
    Migrate = st.button('**Load to MySQL**')
...
# ====================================================   /     Channel Analysis      /   ================================================= #
with tab3:
    st.header(':violet[Channel Data Analysis]')
    st.write ('''(Note:- Analyse the data based on the requested questions from the below list. Select a question to get the insights)''')

    # Check available channel data
    Check_channel = st.button('**Check available channels list**')
    ...
    # -----------------------------------------------------     /   Questions   /    ------------------------------------------------------------- #
    st.subheader(':violet[Channel Analysis ]')
